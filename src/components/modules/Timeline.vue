        ...
        <table class="timeline_canvas">
          <tr class="timeline_canvas_head">
            ...
            >
              ...
              <div
                class="timeline_canvas_day"
                :class="today == date.date ? '-today' : ''"
              >
                {{ date.id }}
              </div>
            </td>
          ...
          <tr v-for="project in projects" :key="project.name">
            ...
            >
              ...
              <div
                ...
                :style="{
                  width: project.fields.period.integerValue * 75 + 'px',
                }"
              ></div>
              <!-- End 案件期間 -->
              ...
              <!-- Start コメント条件分岐 -->
              <div
                v-if="project.fields.comment"
                class="timeline_canvas_comment"
              >
                <div
                  v-if="
                    commentCheck(
                      project.fields.comment.mapValue.fields,
                      date.date
                    )
                  "
                >
...
export default class Timeline extends Vue {
  ...
  public reloadDates(): void {
    this.dates = [] // 初期化
    for (let i = 0; i < dayjs(this.generate).daysInMonth(); i++) {
      // daysInMonth: 月の日数を取得
      this.dates[i] = {
        id: i + 1, // id兼日にち
        date: dayjs(this.generate)
          .startOf('month')
          .add(i, 'day')
          .format('YYYY-MM-DD'),
        weekDay: dayjs(this.generate)
          .startOf('month')
          .add(i, 'day')
          .format('ddd'),
      }
  ...
  public created() {
    // 非同期処理の中ではreturnは書けないので更新はwatchで監視
